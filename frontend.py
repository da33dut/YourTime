...
class App(tk.Tk):
    ...
    def _update_btn_states(self) -> None:
        ...
        takt = cfg.get("takt_seconds", DEFAULT_TAKT_SEC); us = self._t("unit_s")
    ...
    def _on_extend(self) -> None:
        ...
        self.ctrl.extend(takt); self.status_msg("msg_extended", "blue", s=takt)

    def _on_reduce(self) -> None:
        takt = self.ctrl.get_cfg().get("takt_seconds", DEFAULT_TAKT_SEC)
        self.ctrl.reduce(takt); self.status_msg("msg_reduced", "blue", s=takt)
    ...
    def _build_settings(self) -> None:
        """Build day-grid (buttons, range entries, limit spinboxes) and right panel."""
        ...
        # Takt row
        lbl_takt = ttk.Label(right_f, text=self._t("lbl_takt"), width=W_SPINLBL, anchor="w")
        ...
        self._ttk_lw(ttk.Spinbox(right_f, from_=TAKT_SEC_LO, to=TAKT_SEC_HI,
                                  ...
                                  width=W_SPINBOX)).grid(row=2, column=1, pady=PAD_ROW, sticky="w")
    ...
    def _keep_front(self) -> None:
        """Recurring 1-second callback keeping the window in front during the warning state."""
        rem = self.ctrl.get_remaining(); cfg = self.ctrl.get_cfg()
