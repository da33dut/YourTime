...
class LBtn:
    """
    Label-based button with enabled/disabled lock state.

    keep_color=True  → status colour preserved when UI is locked (day-btns, autostart).
    keep_color=False → standard gray-out when disabled (save, reset, quit, action, +/-).
    """

    def __init__(self, parent, text: str, width: int, cmd,
                 active_bg: str = C_GRAY_N, active_fg: str = C_BLACK,
                 bold: bool = False, keep_color: bool = False):
    ...
    def enable(self, on: bool) -> None:
        self._enabled = on
        if on:
            self.w.config(bg=self.active_bg, fg=self.active_fg,
                          cursor="hand2", relief="raised")
        elif self._keep_color:
            self.w.config(bg=self.active_bg, fg=C_DIS_FG,
                          cursor="arrow", relief="flat")
        else:
            self.w.config(bg=C_DIS_BG, fg=C_DIS_FG,
                          cursor="arrow", relief="flat")

    def config(self, **kw) -> None:
        if "active_bg" in kw:
            self.active_bg = kw.pop("active_bg")
            kw["bg"] = self.active_bg if (self._enabled or self._keep_color) else C_DIS_BG
        if "active_fg" in kw:
            self.active_fg = kw.pop("active_fg")
            kw["fg"] = self.active_fg if self._enabled else C_DIS_FG
        self.w.config(**kw)
...
class App(tk.Tk):
    ...
    def _lbtn(self, parent, text, width, cmd,
               ...
               bold=False, lockable=True, keep_color=False) -> "LBtn":
    ...
    def _update_btn_states(self) -> None:
        ...
        takt = cfg.get("takt_seconds", DEFAULT_TAKT_SEC)
        us = self._t("unit_s")
        above = rem != UNLIMITED and rem > takt
        below = rem != UNLIMITED and rem <= takt
        self.btn_reduce.config(text=f"-{takt}{us}" if above else "\u2014")
        self.btn_reduce.enable(above)
        self.btn_extend_btn.config(text=f"+{takt}{us}" if below else "\u2014")
    ...
    def _toggle_autostart(self) -> None:
        try: autostart_set(not autostart_exists())
        except Exception as ex:
            self._refresh_autostart_btn()
            self.status_msg("msg_autostart_err", "red", e=ex); return
        self._refresh_autostart_btn()
        self.status_msg("msg_autostart_on" if autostart_enabled()
                        else "msg_autostart_off", "blue")
    ...
    def _refresh_day(self, day: str) -> None:
        ...
        b.active_bg = col; b.active_fg = C_WHITE
        b.w.config(bg=col, fg=C_WHITE if b._enabled else C_DIS_FG)
    ...
    # --- timer adjust (uses takt as step) ------------------------------------

    def _on_extend(self) -> None:
        s = self.ctrl.get_cfg().get("takt_seconds", DEFAULT_TAKT_SEC)
    ...
    def _tray_setup(self) -> None:
        ...
        self.tray = pystray.Icon("YourTime", img, "YourTime",
                                  pystray.Menu(pystray.MenuItem(
                                      self._t("tray_open"),
                                      lambda *_: self.after(0, self._show), default=True)))
    ...
    def _build_settings(self) -> None:
        ...
        for row, key in enumerate(("row_from","row_to","row_timer","row_limit_min"), start=2):
            lbl = ttk.Label(days_f, text=self._t(key), width=W_ROW,
                            font=FONT_ROW_HDR, anchor="w")
        ...
        for col, (en, short) in enumerate(
                zip(DAYS_EN, self.ctrl.days_short(self._lang)), start=1):
            b = self._lbtn(days_f, short, W_DAY, lambda d=en: self._cycle_day(d),
                            active_bg=DAY_COLORS["on"], active_fg=C_WHITE,
                            bold=True, keep_color=True)
        ...
        ttk.Separator(outer, orient="vertical").grid(
            row=0, column=1, sticky="ns", pady=PAD_BTN)

        right_f = ttk.Frame(outer)
        right_f.grid(row=0, column=2, sticky="new", padx=(8, 4), pady=PAD_BTN)

        # Autostart – keep_color=True
        lbl_as = ttk.Label(right_f, text=self._t("lbl_autostart"),
                           width=W_SPINLBL, anchor="w")
        ...
        self.btn_autostart = self._lbtn(
            right_f, "", W_AUTOSTART, self._toggle_autostart,
            active_bg=C_RED, active_fg=C_WHITE, bold=True, keep_color=True)
        self.btn_autostart.w.grid(row=0, column=1, sticky="w", pady=(0, PAD_BTN))

        # Action – keep_color=False (gray when locked)
        lbl_act = ttk.Label(right_f, text=self._t("lbl_action"),
                            width=W_SPINLBL, anchor="w")
        lbl_act.grid(row=1, column=0, sticky="w", padx=(0, PAD_BTN), pady=(0, PAD_BTN))
        self._reg("lbl_action", lbl_act)
        self.btn_action = self._lbtn(
            right_f, self._t("action_"+self._action), W_ACTION,
            self._cycle_action, active_bg=C_BLUE, active_fg=C_WHITE,
            bold=True, keep_color=False)
        self.btn_action.w.grid(row=1, column=1, sticky="w", pady=(0, PAD_BTN))

        # Takt spinbox (single row, replaces lbl_interval + lbl_extend)
        lbl_takt = ttk.Label(right_f, text=self._t("lbl_takt"),
                             width=W_SPINLBL, anchor="w")
        lbl_takt.grid(row=2, column=0, sticky="w", padx=(0, PAD_BTN), pady=PAD_ROW)
        self._reg("lbl_takt", lbl_takt)
        self.v_takt = tk.IntVar(value=DEFAULT_TAKT_SEC)
        self._ttk_lw(ttk.Spinbox(right_f, from_=TAKT_SEC_LO, to=TAKT_SEC_HI,
                                  textvariable=self.v_takt,
                                  width=W_SPINBOX)).grid(
            row=2, column=1, pady=PAD_ROW, sticky="w")
    ...
    def _build_password_row(self) -> None:
        ...
        fp.pack(side="left", fill="both", expand=True)
        ...
        for col, (key, var) in enumerate(
                [("lbl_pw_new", self.v_pw1), ("lbl_pw_rep", self.v_pw2)]):
    ...
    def _build_buttons(self) -> None:
        ...
        self.btn_save = self._lbtn(bf, self._t("btn_save"), W_SIDE, self.save,
                                    keep_color=False)
        self.btn_save.pack(side="left", padx=(0, PAD_ROW))
        self._reg("btn_save", self.btn_save)
        ...
        self.btn_lock = self._lbtn(bf, self._t("btn_lock"), W_LOCK,
                                    self._on_lock_btn, lockable=False)
        self.btn_lock.w.pack(side="left", fill="x", expand=True,
                              padx=(PAD_ROW, PAD_ROW), ipady=BTN_PAD)
    ...
    def _keep_front(self) -> None:
        rem = self.ctrl.get_remaining(); cfg = self.ctrl.get_cfg()
        warn = rem != UNLIMITED and rem <= cfg.get("takt_seconds", DEFAULT_TAKT_SEC)
        if warn:
    ...
    def _load(self) -> None:
        try:
            ...
            self.btn_lang.config(text=self._lang)
            self.btn_action.config(text=self._t("action_" + self._action))
            # support old key name on first load after migration
            takt = cfg.get("takt_seconds",
                           cfg.get("check_interval_seconds", DEFAULT_TAKT_SEC))
            self.v_takt.set(takt)
    ...
    def save(self) -> None:
        for d in DAYS_EN:
            if self.day_state[d] == "range":
                if (not validate_time(self.day_start[d].get()) or
                        not validate_time(self.day_end[d].get())):
                    self.status_msg("msg_time_invalid", "red"); return
        try:
            self.ctrl.save(
                lang=self._lang, action=self._action,
                takt_sec=int(self.v_takt.get()),
